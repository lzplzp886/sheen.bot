...
export async function POST(request: Request) {
  ...
  try {
    ...
    const pdfBuffer: Buffer = await new Promise((resolve) => {
      ...
      // 大标题：居中显示、加粗
      doc.moveDown(2);
      doc.font("Helvetica-Bold").fontSize(22).text("Sheen Academy Enrollment Form", {
        align: "left",
      });
      doc.moveDown(1);
      ...
      if (formData.selectedTimeslots && formData.selectedTimeslots.length > 0) {
        const scheduleRows: Array<[string, string]> = [
          ["Preferred Slots", formData.selectedTimeslots.join(", ")]
        ];
...
We are excited to have them join our coding and robotics course. Attached is a PDF copy of the enrollment form for your reference. We look forward to seeing your child(ren) soon.

...
export default function CarouselGallery({ images }: CarouselGalleryProps) {
  ...
  const positions = [
    { offset: -2, left: '10%', scale: 0.4, blur: 'blur-sm', zIndex: 1 },
    { offset: -1, left: '25%', scale: 0.6, blur: 'blur-sm', zIndex: 2 },
    { offset:  0, left: '50%', scale: 1.2, blur: '',       zIndex: 3 },
    { offset:  1, left: '75%', scale: 0.6, blur: 'blur-sm', zIndex: 2 },
    { offset:  2, left: '90%', scale: 0.4, blur: 'blur-sm', zIndex: 1 },
  ];

  return (
    <div>
      {/* Main carousel pane */}
      <div className="relative w-full mx-auto h-80 mb-4 bg-extralight rounded-md overflow-visible">
        {positions.map((pos) => {
          ...
          return (
            ...
            >
              <Image
                ...
                alt={images[idx].alt}
                fill
                className={`${pos.blur} object-contain`} />
            </div>
          );
        })}
        {/* nav arrows */}
      ...
      {/* Thumbnails: one scrollable row, always one line, focus updates main */}
      ...
      {/* Lightbox overlay: backdrop closes, image container matches square */}
      {lightboxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-body bg-opacity-80" onClick={closeLightbox}>
